...
HandState StateDetector::classify(const int* flex, const int16_t* imu)
{
  ...
  if (!_middleFlex && flex[2] > config::state::FLEX_MIDDLE_ENTER) _middleFlex = true;
  if ( _middleFlex && flex[2] < config::state::FLEX_MIDDLE_EXIT)  _middleFlex = false;
  ...
  const bool four_curled = _indexFlex && _middleFlex && _ringFlex;

...
enum class FeedbackState { IDLE, SEND_BEEP, WAITING, ACK_BEEP_1, ACK_GAP, ACK_BEEP_2, COLOR_FLASH };
...
void loop()
{
  ...
  if (imu.shouldRead(now)) {
    ...
    uint8_t bits = 0;
    for (int i = 0; i < 5; ++i)
      if (f[i] >= config::flex::THRESHOLDS[i]) bits |= (1u << i);
    s.flex_bits = bits;
    ...
    for (int i = 0; i < 3; ++i) s.accel[i] = d[i];
    for (int i = 0; i < 3; ++i) s.gyro[i]  = d[3 + i];

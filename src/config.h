...
constexpr int OVERSAMPLE = 8; // reads averaged per sample to reduce noise
...
namespace mqtt {
constexpr const char *BROKER = "10.228.192.187";
constexpr const char *BROKER_HOSTNAME = "thinky.local";
...
namespace ble {
  constexpr const char* DEVICE_NAME       = "glove";
  ...
  constexpr int         CONN_MIN_INTERVAL = 16;
  constexpr int         CONN_MAX_INTERVAL = 16;
  constexpr int         CONN_LATENCY      = 0;
  constexpr int         CONN_TIMEOUT      = 600;
} // namespace ble

namespace state {
// Flex thresholds — hysteresis: ENTER > EXIT to prevent boundary oscillation
// Tuned from observed sensor data (post-calibration ADC values)
constexpr int FLEX_THUMB_ENTER = -100; // A0: GRAB when thumb crosses above
constexpr int FLEX_THUMB_EXIT = -150;  // A0: exit GRAB when thumb drops below
constexpr int FLEX_INDEX_ENTER = 600;  // A1: flexed ~1000+, unflexed ~130-440
constexpr int FLEX_INDEX_EXIT = 450;
constexpr int FLEX_RING_ENTER = 600; // A3: flexed ~1055+, unflexed < 540
constexpr int FLEX_RING_EXIT = 450;

// IMU thresholds (raw MPU6050 int16_t, ±2 g scale)
constexpr int POUR_AY_THRESHOLD = 2500; // ay > 2500 → wrist tilted to pour
constexpr int64_t SHAKE_SPREAD_THRESHOLD =
    200000000LL; // max-min of |a|² window

// Debounce: consecutive samples required before state transition (~100 ms at 30
// Hz)
constexpr int DEBOUNCE_SAMPLES = 10;
// How many consecutive non-SHAKE samples needed to exit SHAKE (~500 ms at 30
// Hz)
